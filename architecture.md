...
- Sweep direction (which way Q/E presses rotate each flipper) is a separate `SWEEP_DIRECTION` table, intentionally decoupled from the `MOTOR_DIRECTION` table used for the rest-angle rotation -- flipping one must not flip the other. Current value `{ Left = 1, Right = -1 }` (flipped from the original `{ Left = -1, Right = 1 }` on 2026-08-12 -- see the note below on why this table, not the key mapping, is where a hinge-direction fix belongs).
...
- `CabinetBuilder.build` gives the `"Left"` pivot the NEGATIVE local offset (`-config.FlipperPivotOffsetX`) and `"Right"` the positive one. This is the original assignment -- do NOT change it to try to make the Left/Right code labels match physical side directly; doing so affects the rest of the layout (bumpers/slingshots/etc.) and was reverted twice during testing.
- **Player-facing key mapping is LOCKED and must never be inverted**: Q sends `"Left"`, E sends `"Right"` (gamepad L1/L2 -> `"Left"`, R1/R2 -> `"Right"`), in `FlipperInputController.local.luau`'s `KEY_TO_SIDE`. This is a labeling fact (Q is the physically-left flipper's key), not a tuning knob -- if a hand-built or migration-built cabinet's flippers respond to the wrong key, the defect is in that cabinet's hinge/pivot wiring, and the fix belongs in `SWEEP_DIRECTION` above (or, if only one specific cabinet disagrees with the others, in that cabinet's own flipper construction) -- never here.
- **2026-08-12 history, for context**: this mapping was inverted twice this project (Q="Right"/E="Left") to compensate for hinge-direction bugs on WilliamsCabinet/WilliamsCabinetPlayground, then reverted to natural both times once the actual defect was traced to `SWEEP_DIRECTION` instead. If in doubt about which hinge a keypress is actually driving, use a server-side diagnostic print of the actual pivot `.Name` and `.Position` on keypress, and do a full close-and-reopen of Studio before testing to rule out stale sync -- and always re-verify live, never from code/geometry reasoning alone.
